...
Three Finger Drag is an accessibility-leaning setting for System. Enable dragging windows with 3 fingers (no click needed). Even when the feature was designed with accessibility in mind, it can still be broadly useful because it reduces friction, mechanical effort, or communication overhead.
...
2. Navigate to `Accessibility > Pointer Control > Trackpad Options`, or use Settings search to jump there faster.
3. Change the option deliberately and note the original value first if you might want to revert it later.
4. Check that the behavior now matches the hack: Enable dragging windows with 3 fingers (no click needed).
