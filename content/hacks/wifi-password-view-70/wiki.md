...
WiFi Password View is a settings-driven feature for System. Click the '...' next to a network > Copy Password. Useful for sharing. The value here is persistence: you change the system once, and the new behavior stays available in later sessions until you change it back.
...
2. Navigate to `System Settings > Wi-Fi`, or use Settings search to jump there faster.
3. Change the option deliberately and note the original value first if you might want to revert it later.
4. Check that the behavior now matches the hack: Click the '...' next to a network > Copy Password. Useful for sharing.
